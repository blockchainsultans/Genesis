...
export default class Admintabletable extends React.Component{
  constructor(props){
    super(props);
    this.state = {
      id:"",
...
componentDidMount() {
  ...
  AprooveLoan =() =>{
    ...
    render(){
        ...
        if (this.state.curTime == this.state.curTime){
      ...
      for(var i = 0; i < regUser.length; i++){
        ...
        for(var j = 0; j < loanamount.length; j++){
          ...
          var loanstatus1; 
          if (loanstatus[j]==1){
            loanstatus1 = web3.toUtf8(regUser[loanGivenBy[j]]);
                    ...
                    <table id="dynamic-table" className="table table-striped table-bordered table-hover">
                      <thead>
                        <tr>
                          ...
                          <th className="hidden-480">Loan Status</th>
